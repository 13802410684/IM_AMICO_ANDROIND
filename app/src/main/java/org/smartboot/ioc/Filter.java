 ...
 * 消息/服务过滤器。
 * <p>
 * smart-ioc设计的{@code Filter}与状态机{@link StateMachineEnum}看上去比较类似，但两者是以不同的维度对发生的事件进行Filter处理。
 * </p>
 * <p>
...
public interface Filter<T> {
     ...
     */
    void connected(NioSession<T> session);
     ...
     */
    void closed(NioSession<T> session);
     ...
     */
    public void readFilter(NioSession<T> session, int readSize);
     ...
     * @param msg     编解码后的消息实体
     */
    public void processFilter(NioSession<T> session, T msg);
     ...
     * @param e       本次处理异常对象
     */
    public void processFail(NioSession<T> session, T msg, Throwable e);
     ...
     */
    public void writeFilter(NioSession<T> session, int writeSize);
